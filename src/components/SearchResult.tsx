...
const SearchResult = (props: Props) => {
    ...
    const fetch = () => {
        ...
        for(let i = 0; i <= 5; i++) {
            list.push({
                id: Math.random() * 100,
                title: 'Mamma ho perso l\'aereoMamma ho perso l\'aereoMamma ho perso l\'aereoMamma ho perso l\'aereo',
                description: '',
                coverPath: '',
                genreIds: [1,2,3,4,5],
                posterPath: '',
    ...
    return (
        <div className="absolute left-0 top-[48px] right-0 rounded-md overflow-hidden bg-header">
            {items.map(((film, idx) => {
                return (
                    <div key={idx} className="my-1 flex items-start p-1.5 rounded-lg hover:bg-primary cursor-pointer">
